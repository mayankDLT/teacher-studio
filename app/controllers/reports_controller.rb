=begin
  ...
  Description: Controller file for managing reports
...
class ReportsController < ApplicationController
  ...
  def userReport
   ...
   def generateUser 
        ...
        @examcode = @findExam.exam_code.gsub(/-/, '')
        @examdate = @findExam.exam_date.strftime("%Y%m%d")
        headers["Content-Disposition"] = 
          "attachment; filename=\"#{@findExam.name}_#{@examcode}_#{@examdate}.pdf\""      
  end
  ...
  def overall
     ...
     @exam_type = params[:examtype_id].to_i
     @yrs = Array.new(10){|i| Date.current.year-i}
     @examTypes = Examtype.where(["organization_id = ?",@organization_id])
     dateTime = Time.now()
