=begin
  attend_exams_controller.rb
  Description: Controller file for attending exams module.
...
class AttendExamsController < ApplicationController
  ...
  def calculateScore
   ...
   @categoryexamuser = Categoryexamuser.find_by_categoryexam_id_and_categoryuser_id(categoryExam,categoryUser)
    ...
    sql = "SELECT F.id,S.id,S.name as examname,S.pass_mark as pass_percentage,E.attempt,S.total_mark,Sum(answer_mark) As marks,(Sum(answer_mark) * 100)/S.total_mark as percentage
    From evaluations E
    ...
    unless @categoryexamuser.need_evaluation == 1
        @exam.each do|e|
          ...
          if percentage.to_f >= e.pass_percentage.to_i
           @examResult.status = 'p'
