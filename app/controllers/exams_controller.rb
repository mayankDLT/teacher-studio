=begin
  exams_controller.rb
  Description: Controller file for managing exams.
         ...
         if @markAssigned > @examMark
           ...
           flash[:notice] = "#{t('flash_notice.ques_mark_exceeding')} #{@markDifference.to_i}."
            ...
            if @markAssigned == @examMark
              ...
              fetchMark
              @mark_less_by = @examMark - @markAssigned
              flash[:success] = t('flash_success.ques_added')
              flash[:notice] = "#{t('flash_notice.need_to_add_questions_head')} #{@mark_less_by} #{t('flash_notice.need_to_add_questions_tail')}"
            end
         end
  ...
  def updateExamtype
    ...
    def evaluate      
      ...
      unless params[:evaluate] == nil
        ...
        if (eval_qstn.parent_id != nil && eval_passage == nil) || @eval_flag == 0
          @manualflag = "qs_al_eval"
        else
          @manualflag = "m"
        end
      end
