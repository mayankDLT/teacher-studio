=begin
  ...
  Description: Controller file for managing the questions and answers.
      ...
      @seq = 1
        unless params[:fields] == nil        
         params[:fields].each do |i, values|
          ...
          @question = Question.new
          @question.description = format_desc(params[:description])
          @question.question_hint = params[:hint]
  ...
  def update    
    if params[:qt_id] == "1" or params[:qt_id] == "2" or params[:qt_id] == "4" or params[:qt_id] == "5"
     ...
     @last_image = Image.find(:last)
      ...
      @question.description = format_desc(params[:question][:description])
...
def publish_unpublish
 ...
 def sharewithCategory
  ...
  def format_desc string
    return string.gsub(/width:[^;]*;/, 'width: auto;')
  end
end
