class UserMailer < ActionMailer::Base
  ...
  @@host_name = Setting.find(:first).url
   def user_creation_confirmation(user,pwd)  
     ...
     if t('language.lang') == "en"
      ...
      @email_content = @email_template.content_ar
     elsif t('language.lang') == "zh"
      @email_content = @email_template.content_zh
     end
   ...
   def send_reset_password(user)
     ...
     @replace_url_string = "<a target='_blank' href='#{@edit_password_reset_url}'>#{t('email.pass_reset')}</a>"
     @email_content = @email_content.gsub("$^_username_^$",@@user_name).gsub("$^_reset_url_^$",@replace_url_string)
   ...
   def user_registration_email_confirmation(user,pwd)
     ...
     @replace_url_string = "<a target='_blank' href='#{@confirm_email_user_registration_url}'>#{t('email.verify')}</a>"
     @email_content = @email_content.gsub("$^_username_^$",@@user_name).gsub("$^_userlogin_^$",@@user_login).gsub("$^_verify_email_^$",@replace_url_string)#.gsub("$^_userpassword_^$",@@user_password)
  ...
  def user_set_password(user)
     ...
     @set_password = verifyPassword_password_reset_url(user.perishable_token, :host => @host_name)
     @replace_url_string = "<a target='_blank' href='#{@set_password}'>#{t('email.verify_set_pass')}</a>"
     @email_content = @email_content.gsub("$^_username_^$",@@user_name).gsub("$^_userlogin_^$",@@user_login).gsub("$^_verify_and_set_password^$",@replace_url_string)
