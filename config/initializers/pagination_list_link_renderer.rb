class PaginationListLinkRenderer < WillPaginate::LinkRenderer
 ...
 def to_html
  ...
  #  links.push(page_link_or_span(@collection.next_page, 'next', @options[:next_label]))
    ...
    html = page_link_or_span(@collection.previous_page, 'previous', @options[:previous_label]) + links.join(@options[:separator]).html_safe + page_link_or_span(@collection.next_page, 'next', @options[:next_label])
